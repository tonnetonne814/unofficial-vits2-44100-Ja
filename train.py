...
def run(rank, n_gpus, hps):
  ...
  if hps.train.is_finetune is True:
    try:
      _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.train.finetune_model_dir, "G_finetune.pth"), net_g, optim_g)
      _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.train.finetune_model_dir, "D_finetune.pth"), net_d, optim_d)
      if net_dur_disc is not None:
        _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.train.finetune_model_dir, "DUR_finetune.pth"), net_dur_disc, optim_dur_disc)
  ...
  # save last model
  utils.save_checkpoint(net_g, optim_g, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "G_last_step.pth"))
  utils.save_checkpoint(net_d, optim_d, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "D_last_step.pth"))
  if net_dur_disc is not None:
    utils.save_checkpoint(net_dur_disc, optim_dur_disc, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "DUR_last_step.pth"))
